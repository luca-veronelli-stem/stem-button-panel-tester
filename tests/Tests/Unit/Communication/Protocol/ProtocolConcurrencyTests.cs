    ...
    public class ProtocolConcurrencyTests
    {
        ...
        public class NetworkLayerConcurrency
        {
            ...
            public async Task Create_ConcurrentCalls_AllPacketsValid()
            {
                ...
                // Act
                IEnumerable<Task> tasks = Enumerable.Range(0, taskCount).Select(i => Task.Run(() =>
                {
                ...
                foreach (List<byte[]> chunks in allPackets)
                {
        ...
        public class ReassemblerConcurrency : IDisposable
        {
            ...
            public async Task ProcessReceivedChunk_ConcurrentSingleChunks_AllReassembled()
            {
                ...
                IEnumerable<Task> tasks = Enumerable.Range(0, packetCount).Select(i => Task.Run(() =>
                {
                    byte[] packet = CreateTransportPacket((byte)(i % 256));
                    expectedPackets.Add(packet);

                    int packetId = (i % 7) + 1; // Cycle through valid IDs
                    byte[] chunk = CreateChunkWithNetInfo(packet, remainingChunks: 0, packetId: packetId);
            ...
            public async Task ProcessReceivedChunk_ConcurrentMultiChunkStreams_AllReassembled()
            {
                ...
                IEnumerable<Task> tasks = Enumerable.Range(1, streamCount).Select(packetId => Task.Run(() =>
                {
                    byte[] packet = CreateLargerTransportPacket(50 + packetId * 10);
                    expectedPackets[packetId] = packet;

                    List<byte[]> chunks = SplitIntoChunks(packet, chunkSize: 8, packetId: packetId);
                ...
                foreach (byte[] expected in expectedPackets.Values)
                {
            ...
            public async Task ClearReassemblyState_WhileProcessing_NoExceptions()
            {
                ...
                var processingTask = Task.Run(async () =>
                {
                    while (!cts.Token.IsCancellationRequested)
                    {
                        try
                        {
                            byte[] packet = CreateTransportPacket(0x42);
                            byte[] chunk = CreateChunkWithNetInfo(packet, 0, 1);
                            _reassembler.ProcessReceivedChunk(chunk);
                            await Task.Delay(1);
                        }
        ...
        public class ProtocolManagerConcurrency
        {
            ...
            public async Task BuildPackets_ConcurrentCalls_AllSucceed()
            {
                ...
                IEnumerable<Task> tasks = Enumerable.Range(0, taskCount).Select(i => Task.Run(() =>
                {
                    ...
                    List<NetworkPacketChunk> packets = manager.BuildPackets((ushort)(0x0100 + i), payload, (uint)i, (uint)i, 8);
        ...
        public class PacketIdGeneratorConcurrency
        {
            ...
            public async Task GetNextPacketId_HighContention_AllIdsValid()
            {
                ...
                IEnumerable<Task> tasks = Enumerable.Range(0, taskCount).Select(_ => Task.Run(() =>
                {
            ...
            public async Task GetNextPacketId_ParallelAccess_NoMissedUpdates()
            {
                ...
                // Act
                IEnumerable<Task> tasks = Enumerable.Range(0, iterations).Select(_ =>
        ...
        public class LayerConcurrency
        {
            ...
            public async Task TransportLayer_ConcurrentParse_AllValid()
            {
                ...
                // Act
                IEnumerable<Task> tasks = transportPackets.Select(packet => Task.Run(() =>
                {
