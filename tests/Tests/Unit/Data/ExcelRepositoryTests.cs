...
namespace Tests.Unit.Data
{
    public class ExcelRepositoryTests
    {
        ...
        public async Task GetProtocolDataAsync_Stream_EmptyStream_ReturnsEmptyData()
        {
            ...
            StemProtocolData result = await _repository.GetProtocolDataAsync(emptyStream);
        ...
        {
            using Stream stream = LoadRealExcelStream();

            StemProtocolData result = await _repository.GetProtocolDataAsync(stream);
        ...
        public async Task GetProtocolDataAsync_Stream_MissingSheet_ThrowsInvalidOperationException()
        {
            ...
            InvalidOperationException ex = await Assert.ThrowsAsync<InvalidOperationException>(() => _repository.GetProtocolDataAsync(stream));
        ...
        {
            Exception ex = await Assert.ThrowsAnyAsync<Exception>(() => _repository.GetProtocolDataFromFileAsync(filePath));
        ...
        {
            StemProtocolData result = await _repository.GetProtocolDataFromFileAsync(RealExcelFilePath);
        ...
        {
            using var emptyStream = new MemoryStream();
            StemProtocolData result = await _repository.GetDictionaryAsync(emptyStream, 123);
        ...
        public async Task GetDictionaryAsync_Stream_ValidDataWithMatchingId_ExtractsVariables()
        {
            ...
            StemProtocolData result = await _repository.GetDictionaryAsync(stream, 65601); // 0x00010041 decimal = 65601
        ...
        public async Task GetDictionaryAsync_Stream_NoMatchingId_ReturnsEmptyVariables()
        {
            ...
            StemProtocolData result = await _repository.GetDictionaryAsync(stream, 999); // Non-matching ID
        ...
        {
            using XLWorkbook workbook = CreateWorkbookFromRealData(noHighlight: true);
            using var stream = new MemoryStream();
            workbook.SaveAs(stream);
            stream.Position = 0;

            StemProtocolData result = await _repository.GetDictionaryAsync(stream, 65601);
        ...
        {
            Exception ex = await Assert.ThrowsAnyAsync<Exception>(() => _repository.GetDictionaryFromFileAsync(filePath, recipientId));
            Assert.Contains(expectedMessage, ex.Message);
        }
        ...
        private XLWorkbook CreateWorkbookFromRealData(bool noHighlight = false)
        {
            ...
            IXLWorksheet addrWs = wb.AddWorksheet("Indirizzo protocollo stem");
            ...
            IXLWorksheet cmdWs = wb.AddWorksheet("COMANDI");
            ...
            IXLWorksheet varWs = wb.AddWorksheet("DATI LOGICI SHERPA SLIM");
            ...
            if (!noHighlight)
            {
                row5.Cell("A").Style.Fill.BackgroundColor = XLColor.FromArgb(-7155632);
            }

            IXLRow row6 = varWs.Row(6);
            ...
            if (!noHighlight)
            {
                row6.Cell("A").Style.Fill.BackgroundColor = XLColor.FromArgb(-7155632);
            }

            IXLRow row7 = varWs.Row(7);
            ...
            if (!noHighlight)
            {
                row7.Cell("A").Style.Fill.BackgroundColor = XLColor.FromArgb(-7155632);
            }
