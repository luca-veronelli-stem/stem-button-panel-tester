...
- **Dictionary credential — stopgap.** ⚠️ The DPAPI-backed credential store and the first-run bootstrap step are currently **bypassed**. The API key is read directly from `Dictionary:ApiKey` (override via env var `Dictionary__ApiKey`; supplier-specific value lives in gitignored `appsettings.Production.json`) and sent as `X-Api-Key` to match the shared `stem-dictionaries-manager` deployment used by `stem-device-manager`. This violates FR-011a/b/c and Constitution Principle I (`CONFIGURATION` standard) — see [`docs/STOPGAP_API_KEY.md`](./docs/STOPGAP_API_KEY.md) for scope, waived requirements, and the re-secure plan. `DpapiCredentialStore.cs` and the `IInstallationCredentialStore` F# contract are retained on disk so the re-secure PR is a recompose, not a re-implement.
